...
- **Dependencies.** `bitcoin`, `bitcoin_hashes` and `secp256k1` are the only third-party
  crates the node links, pinned exactly ([decision 0001](docs/decisions/0001-primitive-crates.md));
  `bitcoinconsensus` is a test oracle for `crates/differential` alone
  ([decision 0003](docs/decisions/0003-differential-oracle.md)). Any new crate, including dev
  and build dependencies, needs a recorded decision in `docs/decisions/` first. Never enable
