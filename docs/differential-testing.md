...
| Size | many records per run (one per spender per input-count pass; exact count unverified); it is the single richest taproot/tapscript consensus corpus Core has. Producing it needs a working Core functional-test build (Python framework + a `bitcoind` with wallet); the installed `bitcoind` can be pointed at with `BITCOIND=/run/current-system/sw/bin/bitcoind` (section 4.1) but the test also needs a `config.ini` from a build tree | `feature_taproot.py: skip_test_if_missing_module` (`skip_if_no_wallet`) |
