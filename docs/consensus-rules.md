...
| `0xb2 OP_CHECKSEQUENCEVERIFY` (`NOP3`) | NOP before CSV; after: 5-byte operand, `>= 0`; if operand bit 31 set = NOP; else `tx.version >= 2`, input bit 31 clear, same type (bit 22), masked operand `<=` masked input | same | `EvalScript`; `CheckSequence`; [BIP112] |
...
| OP_CODESEPARATOR in scriptCode | serialized scriptCode has all `OP_CODESEPARATOR`s removed (BASE and v0) | C | `CTransactionSignatureSerializer::SerializeScriptCode` |
