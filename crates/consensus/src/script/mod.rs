...
//! over a shared [`TxPrecomputed`] (BM-D2 decision 4). [`sigop_count`], [`p2sh_sigop_count`]
//! and [`witness_sigop_count`] are the static scans behind the block-level signature
//! operation budget, Core's `GetSigOpCount`, `GetP2SHSigOpCount` and `CountWitnessSigOps`.
//!
...
pub use interpreter::MAX_SCRIPT_SIZE;
